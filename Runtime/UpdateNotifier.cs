...
namespace Core.UnityUtils
{
    ...
    public class UpdateNotifier : SingletonBaseMonoBehaviour<UpdateNotifier>
    {
        ...
        public void Register(IUpdateHandler updateHandler)
        {
            ...
            {
                Debug.Log("Duplicate of handlers");
                return;
            }
#endif
        ...
        private void UnregisterDeadUpdateNotifiers<T>(List<T> list, int from, Action<T> onRemove = null)
            where T : IUpdateHandler
        {
        ...
        private void ClearDeadUpdateNotifiers<T>(List<T> list, int from, Action<T> onRemove = null)
            where T : IUpdateHandler
        {
            var removesPerFrame = 3;
